...
$data = "SELECT * FROM `employee` WHERE `employee_id` = '$employeeids'";
$query = mysqli_query($conn, $data);
while ($fetch = mysqli_fetch_assoc($query)) {
    $id = $fetch['employee_id'];
    $employeenames = $fetch['employee_name'];
    $employeeemail = $fetch['employee_email'];
    $employeemobile = $fetch['employee_phone_no'];

    global $employeenames;
    global $employeeemail;
    global $employeemobile;
}
$data = "SELECT * FROM `login` WHERE `login_employee_id` = '$employeeids'";
$query = mysqli_query($conn, $data);
while ($fetch = mysqli_fetch_assoc($query)) {
...
<head>
    ...
    <title>Laundry Admin - Edit employee</title>
                        ...
                        <div class="col-lg-10 col-md-10 col-xs-12 col-sm-12" style="background-color: white;padding:20px;">
                            ...
                            <form action="" method="POST">
                                ...
                                <div class="form-group row">
                                    <label for="inputEmail3" class="col-sm-2 col-form-label">Employee  Names</label>
                                    <div class="col-sm-10">
                                        <input type="text" class="form-control" id="inputEmail3" name="user_name" value="<?php echo $employeenames; ?>">
                                    </div>
                                </div>
                                <div class="form-group row">
                                    <label for="inputEmail3" class="col-sm-2 col-form-label">Email Address</label>
                                    <div class="col-sm-10">
                                        <input type="email" class="form-control" id="inputEmail3" name="user_email" value="<?php echo $employeeemail; ?>">
                                    </div>
                                </div>
                                ...
                                <div class="form-group row">
                                    ...
                                    <div class="col-sm-10">
                                        <input type="number" class="form-control" id="inputEmail3" name="user_mobile_no" value="<?php echo $employeemobile; ?>">
                                    </div>
                                </div>
                                <div class="form-group row">
                                ...
                                <div class="form-group row">
                                    ...
                                    <div class="col-sm-10">
                                        <button type="submit" class="btn btn-warning btn-block text-light" name="registeruser">Update Employee Account</button>
                                    </div>
                                </div>
                                <?php
                                ...
                                if (isset($_POST['registeruser'])) {
                                    ...
                                    if (empty($names) || empty($email) ||  empty($mobileno) ||  empty($username)) {
                                        ...
                                        $insertuser = "UPDATE  `employee` SET `employee_name`='$names', `employee_email`='$email', `employee_phone_no`='$mobileno' WHERE `employee_id`='$employeeids'";
                                        $queryinsertuser = mysqli_query($conn, $insertuser);

                                        if ($queryinsertuser) {
                                            $lastid = mysqli_insert_id($conn);
                                            $password = $username;
                                            $insertlogin = "UPDATE `login` SET `login_username`='$username' WHERE `login_employee_id`='$employeeids'";

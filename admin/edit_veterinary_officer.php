...
$data = "SELECT * FROM `veterinary_officers` WHERE `veterinary_officers_id` = '$customerids'";
$query = mysqli_query($conn, $data);
while ($fetch = mysqli_fetch_assoc($query)) {
    $id = $fetch['veterinary_officers_id'];
    $customernames = $fetch['veterinary_officers_officer_name'];
    $customeremail = $fetch['veterinary_officers_email_address'];
    $customermobile = $fetch['veterinary_officers_contact_number'];
    $farmerlocation = $fetch['veterinary_officers_address'];
...
$data = "SELECT * FROM `login` WHERE `login_officer_id` = '$customerids'";
$query = mysqli_query($conn, $data);
while ($fetch = mysqli_fetch_assoc($query)) {
...
<head>
    ...
    <title>Editing Veterinary Officer Account</title>
                        ...
                        <div class="col-lg-10 col-md-10 col-xs-12 col-sm-12" style="background-color: white;padding:20px;">
                            ...
                            if (isset($_POST['register_btn'])) {
                                ...
                                if (empty($names) || empty($email) || empty($mobileno) || empty($username) || empty($farmlocation) || empty($password)) {
                                    ...
                                    $customerids = $_GET['id'];
                                    $updateuser = "UPDATE `veterinary_officers` SET `veterinary_officers_officer_name`='$names', `veterinary_officers_address`='$farmlocation', `veterinary_officers_contact_number`='$mobileno', `veterinary_officers_email_address`='$email' WHERE `veterinary_officers_id`='$customerids'";
                                    $queryupdateuser = mysqli_query($conn, $updateuser);

                                    if ($queryupdateuser) {
                                        $password = md5($password);
                                        $updatelogin = "UPDATE `login` SET `login_username`='$username', `login_password`='$password' WHERE `login_officer_id`='$customerids'";
                            ...
                            <form action="" method="POST" autocomplete="off">
                                <center><span>Edit Veterinary Account </span></center>
                                <hr>
                                <?php
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" name="location_address" value="<?php echo isset($_POST['location_address']) ? htmlspecialchars($_POST['location_address']) : $farmerlocation; ?>" class="form-control">
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="form-group">
                                        <button type="submit" class="btn btn-block btn-danger" name="register_btn">Update Veterinary Account</button>
                                    </div>
                            </form>
                        </div>
                        <div class="col-lg-1 col-md-1 col-xs-12 col-sm-12"></div>
